...
| `CLASSIFICAÇÃO` | `ClassificacaoController` | Tela de classificação de movimentos de caixa. `LISTAR PENDENTES DE CLASSIFICACAO` (`/classificacao/movimentos`) traz os lançamentos de `movto_caixa` que ainda não têm item de pagamento e/ou plano financeiro atribuído (mesmos filtros de `CAIXA E BANCOS > LISTAR MOVIMENTOS`), já com `idpessoa`/nome do cliente quando o lançamento já tiver esse vínculo — vincular pessoa não é feito por aqui, é outra tela. `CLASSIFICAR MOVIMENTO` (`/classificacao/movimentos/:idmovtocaixa`, PUT) grava `iditempagamento`/`idplanofinanceiro`; os dois campos do body são opcionais mas ao menos um é obrigatório — o lançamento só sai da lista de pendentes quando os dois estiverem preenchidos. |
